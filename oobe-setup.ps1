# oobe-setup.ps1
# OEM Windows 11 24H2 bootstrap for end users – minimal prompts, full progress & logging
...
# --------------------------
# 1) Local admin GPAdmin2 (no password, no prompt)
# --------------------------
try {
  ...
  Write-Log ("Verify local admin present+admin: {0}" -f $adminOk) ($adminOk ? "SUCCESS" : "ERROR")
...
try {
  ...
  Write-Log ("OOBE disabled; verify: {0}" -f $oobeOk) ($oobeOk ? "SUCCESS" : "ERROR")
} catch { Write-Log "Failed to set OOBE flags: $($_.Exception.Message)" "ERROR"; throw }

# --------------------------
# 3) Pin to Windows 11 24H2 (TargetReleaseVersion)
# --------------------------
try {
  ...
  Write-Log ("Configured TRV for Windows 11 24H2. Verify: {0}" -f $trvOk) ($trvOk ? "SUCCESS" : "ERROR")
...
# --------------------------
# 5) Prompt USER for WS1 username/password (kept in memory only)
# --------------------------
Write-Host ""
...
# Convert to clear text for MSI only, then zero later
...
# Fixed tenant details (as requested)
$WsServer  = "https://ds1106.awmdm.com"
$WsGroupId = "DataTechAILandingO"

# --------------------------
# 6) Download Workspace ONE Intelligent Hub with visible progress
# --------------------------
...
$downloadUrl = "https://packages.omnissa.com/wsone/AirwatchAgent.msi"   # primary
    ...
    while (($read = $in.Read($buffer,0,$buffer.Length)) -gt 0) {
      ...
      if ($sw.Elapsed.TotalMilliseconds -ge 250) {
        ...
        $status = if ($total) {
          ...
          ("{0:N1} MB of {1:N1} MB" -f ($totalRead/1MB), ($total/1MB))
        } else {
          ("{0:N1} MB downloaded" -f ($totalRead/1MB))
        }
...
try {
  ...
  # Fallback via getwsone landing + same package endpoint
  try {
...
try {
  ...
  $msiArgs = @(
    ...
    "USERNAME=$WsUser",
    "PASSWORD=$WsPwdPlain",
    "ASSIGNTOLOGGEDINUSER=Y"   # keep last as per vendor guidance
  )
  ...
  # Zero the plaintext password from memory ASAP
...
Write-Log ("Verify Hub files present: {0}" -f ($hubOk -gt 0)) (($hubOk -gt 0) ? "SUCCESS" : "WARN")
